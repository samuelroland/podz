...
<div {{ $attributes->whereDoesntStartWith('wire:model')->merge(['class' => 'mt-3']) }}>
    ...
    <div>
        ...
        <div x-data="{ isUploading: false, progress: 0, failed: false, uploadCompleted: false }" x-on:livewire-upload-start="isUploading = true; uploadCompleted = false" x-on:livewire-upload-error="failed = true" x-on:livewire-upload-progress="progress = $event.detail.progress" x-on:livewire-upload-finish="uploadCompleted = true">
            ...
            @if ($type != 'textarea')
            <input {{ $attributes->whereStartsWith('wire:model') }} placeholder="{{ $placeholder }}" type="{{ $type }}" value="{{ old($name) }}" class="{{ $cssOnField }} rounded-md px-1 border border-gray-400 {{ in_array($type, ['checkbox']) ? '' : 'h-8' }}" name="{{ $name }}" {{ $type=='file' ? '\@change="isUploading = false; failed = false"' : '' }} />
            @else
            ...
            <div x-show="isUploading" x-cloak>
                Progression: <span class="text-green" x-show="!failed" x-text="uploadCompleted ? 'Téléversé!' : 'En cours'"></span>
